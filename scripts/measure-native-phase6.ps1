...
$exe = Join-Path $repo 'target/release/examples/native_input_state.exe'
$output = Join-Path $repo 'target/phase-06-release'
if (-not (Test-Path $exe)) {
    throw "找不到 Release 验收程序：$exe，请先构建 native_input_state example"
}
...
if ($null -eq $exitCode -and $stdoutText -match 'Phase 6 Native Input State') { $exitCode = 0 }
if ($exitCode -ne 0 -or $stdoutText -notmatch 'Phase 6 Native Input State') {
    throw "Phase 6 验收程序失败，退出码 $exitCode，见 $output"
}
$result = [pscustomobject]@{
    scenario = 'Phase 6 Input State；500/1000/2000/4000/8000 MouseMove 压力、离散事件完整性、Wake 合并'
    repeats = $Repeats
